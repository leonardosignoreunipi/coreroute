...
# unused now that this module is only imported (see below); kept for the
# commented-out code further down, which still refers to them
# import os
# import subprocess
# import tempfile
# import itertools
...
# SEEDS/SIZES/FLOW_FACTORS/PCT_MODS/EPOCHS/MODES: only used by _run_batch and
# __main__ below, both unused now (superseded by drift_bench.py) -- commented
# out together with them. TOPOLOGIES/DEGRADE_FACTOR/STRATEGY stay active:
# TOPOLOGIES is imported by heuristic_benchmark.py, DEGRADE_FACTOR is used by
# _perturb_epoch, STRATEGY is used by _build_system.
# SEEDS          = [224737, 742073, 871871, 350377, 1003001, 1234567, 15485863, 990635, 782715, 510655]
...
_ROW_DEFAULTS = {
    ...
    "T": None, "T_generate_candidates": None, "T_prolog_strategy": None,
    "N_KO": None, "N_R": None,
    "P_KO": None, "P_R": None,
    "n_links_epoch": None, "frac_rr_degraded": None,
    "diff_simm_tot": None, "flows_changed": None, "avg_latency": None,
    "no_path_count": None,
    "ok": False, "error": "",
}
...
def _run_epoch(network, kb, engine, ctrl, rr_edges, pct_mod, rng, reasoning_fn) -> dict:
    """One drift epoch: perturb, then measure ONE reasoning mode (CR's
    continuous_reasoning or FULL's full_recompute) -- this batch runs only
    one mode, so its result simply persists in the KB, no snapshot/restore
    needed (see module docstring for why splitting CR/FULL into independent
    batches loses no comparability).

    Returns:
        Dict of this epoch's metrics (timing, KO/rerouted counts,
    ...
    t, (_, n_ko, n_r, no_path_count) = _timed(reasoning_fn)
    t_gen, t_prolog = engine.last_time_strategy, engine.last_time_prolog_strategy
    metrics = _compute_metrics(pre, _active_routes(kb), engine)

    return {
        "T": t,
        "T_generate_candidates": t_gen, "T_prolog_strategy": t_prolog,
        "N_KO": n_ko, "N_R": n_r,
        "n_links_epoch": n_links,
        "frac_rr_degraded": frac,
        "no_path_count": no_path_count,
        "diff_simm_tot": metrics["diff_simm_tot"],
        "flows_changed": metrics["flows_changed"],
...
# --- Everything below is UNUSED: superseded by drift_bench.py (Ray Tune).
# _run_batch here returns a list for a hand-rolled ray.remote(max_calls=1)
# sliding-window scheduler; drift_bench.py has its own _run_batch that calls
# tune.report() per epoch instead, plus the fixed failure-padding behavior
# (see that file). Kept only for reference, never executed.
#
# def _run_batch(config_base: dict) -> list:
#     """Run all pct_mods (each for EPOCHS cumulative-drift epochs) of one
#     (topology, n, flow_factor, seed, strategy) combination, strategy being
#     CR (continuous_reasoning) or FULL (full_recompute). INIT is done once;
#     each pct_mod restarts from the post-init state, then drifts over its
#     epochs -- one trajectory of this batch's strategy alone.
#
...
#     strategy    = config_base.get("strategy", MODES[0])
#     num_flows   = int(flow_factor * n)
#     pct_mods    = config_base.get("pct_mods", PCT_MODS)
#     epochs      = config_base.get("epochs", EPOCHS)
#
#     base = {"strategy": strategy, "path_strategy": STRATEGY, "topology": topology, "n": n, "flow_factor": flow_factor, "seed": seed, "init_routed": None}
#     results   = []
#     topo_file = None
#
#     try:
...
#         reasoning_fn = ctrl.continuous_reasoning if strategy == "CR" else ctrl.full_recompute
#
...
#
#     all_configs = [
#         {"topology": t, "n": n, "flow_factor": ff, "seed": s, "strategy": strat}
#         for t, n, ff, s, strat in itertools.product(TOPOLOGIES, SIZES, FLOW_FACTORS, SEEDS, MODES)
#     ]
...
#     print(f"commit={commit} | MODES={MODES} | STRATEGY={STRATEGY} | SEEDS={SEEDS} | SIZES={SIZES} " f"| FLOW_FACTORS={FLOW_FACTORS} | PCT_MODS={PCT_MODS} | DEGRADE_FACTOR={DEGRADE_FACTOR}")
...
#               f"strat={b0['strategy']} topo={b0['topology']} n={b0['n']} ff={b0['flow_factor']} seed={b0['seed']} "
